function [G_R_T, G_T_R] = importS2P_1(f, Folder)
...
for n = 1 : length(listing)
    ...
    if i == 0
        ...
        g_R_T = powergain(sobj_R_T, 50, 'Gp');
        G_R_T.mid(j) = 10.*log10(g_R_T(I));
        
        g_T_R = powergain(sobj_T_R, 50, 'Gp');
        G_T_R.mid(j) = 10.*log10(g_T_R(I));
        
    else
        
        g_R_T = powergain(sobj_R_T, 50, 'Gp');
        G_R_T.grid(i,j) = 10.*log10(g_R_T(I));
        
        g_T_R = powergain(sobj_T_R, 50, 'Gp');
        G_T_R.grid(i,j) = 10.*log10(g_T_R(I));
