function w = Wigner3j(j1,m1,j2,m2,j,m)
...
% Selection rules
if ( j > (j1 + j2) ) || ( j < abs(j1 - j2) ) ... % j3 out of interval
   || ( m1 + m2 + m ~= 0 ) ... % non-conserving angular momentum
...
w = sum( (-1).^t .* exp( -ones(1,6) * gammaln( [t; t-t1; t-t2; t3-t; t4-t; t5-t] +1 ) + ...
    gammaln( [j1+j2+j+1, j1+j2-j, j1-j2+j, -j1+j2+j, j1+m1, j1-m1, j2+m2, j2-m2, j+m, j-m] +1 ) ...
    * [-1; ones(9,1)] * 0.5 ) ) * (-1)^( j1-j2-m );
